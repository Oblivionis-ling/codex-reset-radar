...
日期：2026-09-21。整体：**PARTIAL**。代码修复、隔离回归、真实 Backend / API 联调已完成；用户刷新后，21:45 已完成 Profile / Replies 恢复、各三次持续心跳及采集活动验收。视觉验收仍待核验，远端发布未执行。下文 17 时记录为修复现场，最新状态以第 10 节为准。
...
## 10. 用户刷新后的恢复验收（21:45 更新）

用户确认已刷新两页后，仅使用只读健康、Radar、Web 代理接口和有限日志尾部核验，没有额外触发模型、重启服务或修改生产记录。

| 采集路 | 同一实例下三次实际观察时间（北京时间） | sequence | 结果 |
| --- | --- | --- | --- |
| Profile | 21:43:32.915 / 21:44:24.929 / 21:45:24.935 | 9 / 11 / 13 | FRESH，跨度 112 秒 |
| Replies | 21:43:35.405 / 21:44:32.934 / 21:45:32.944 | 12 / 14 / 16 | FRESH，跨度 117 秒 |
| Search | 最近 21:43:44.310 | 1（当前实例） | FRESH，单独核验 |

两路心跳接收时间紧随观察时间，并非旧消息集中补传。21:44:24、21:45:24 的批次各含 9 条帖子；21:44:33、21:45:33 的批次各含 5 条，均成功入库或去重，数据库 last_seen_at 同步推进。这是已有真帖扫描，不是新帖发现延迟测试。

采集恢复的两个健康转换触发被合并为一次真实 Judge 请求：**235**，21:43:28 生成、21:43:35 完成，有效至 **23:43:28**；Prompt 仍为 `v2-reset-judge-8-context-health`，24 个输入版本，校验 VALID。生成时健康和当前健康均为 HEALTHY；display_mode=current，等级依次为 **GREEN / GREEN / GREEN / YELLOW**。Backend 与 Web 代理完整 Radar 响应一致。特殊预告仍独立返回，不产生正式事件。

调用账本补充：上次验收后还发生 3 次正常 hourly 调度（Judge 232—234），加本次健康恢复 1 次（235），均首请求成功、无重试。本跟进手动调用 0；连同初始受控调用，已观察到修复运行累计 5 次真实 Judge 请求，不能仍宣称整个运行期只有 1 次。

数据库 integrity_check=ok、外键错误 0；正式事件及周期与 before.db 逐行比较不变。Backend / Web 持续可访问，加载指纹仍为 `c9b1fd210a8ee0e1`。微信 0、邮件 0。保留原始 acceptance.json 不覆盖，新证据为 `D:\work\20260828-CodexResetRadar\runtime\judge-health-fix-20260921\recovery-acceptance.json`。

第 9 节的采集停点现已解除。尚未完成的仅包括本轮页面视觉验收及远端发布；原断连的浏览器侧根因未被证实，刷新恢复不等于证明其根因。既有父帖失败记录继续保留。未再次运行无代码变化的全套回归，也未额外付费重判。
