...
typed J14 的失败时长与无合法回退已保留，但拒绝后的本地原始选项没有随二次回退异常落盘；
因此其原始选项不计入 0/8 的可审查子集，仍留在全量失败分母，不能声称所有原始答案已完整保存。
...
最终工作树及仅含已提交源码的检出均为 **Backend 108 项通过**；Web 7、Collector 18 项及其
类型检查/构建通过。干净空数据库的正式 Uvicorn API 在隔离 8799 端口启动、健康和 Radar 读取通过，
不加载模型、不触发付费调用，测试进程已回收。`clean-smoke.json` 记录 `OFFLINE_PASS`。
功能提交 `318bf962bb0aebef2652b130b924065bbc3c9bb4` 已正常推送，
[草稿 PR #7](https://github.com/Oblivionis-ling/codex-reset-radar/pull/7) 的 CI run `35709879929`
三项均通过（Backend 26s、Web 14s、Collector 13s）。
远端 main 仍为 `2f6f27bb3ccf80a84a9fa6818596b528358fe75c`，没有合并 Laya、没有新标签。
...
最终只读检查：生产事件与周期内容哈希均与快照一致，数据库完整性/外键正常；
Backend、Web（HTTP 200）和 Profile/Replies/Search 保持健康。
已加载核心指纹仍为 `c9b1fd210a8ee0e1`、应用 Alpha 4，最新生产 Judge #265 校验有效。
注意旧健康接口的 `commit` 是动态读取工作树 HEAD，不代表进程已重载；本轮按加载指纹核实，
没有把功能分支提交号冒充已加载新版本。生产未重启，仍运行原 DeepSeek-only。
采集自然继续，不要求生产帖子数量固定为快照的 405 条。
